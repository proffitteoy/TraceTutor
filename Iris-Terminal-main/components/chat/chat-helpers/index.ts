...
export const handleMemoryRetrieval = async (
  userInput: string,
  _workspaceId: string,
  embeddingsProvider: "openai" | "local",
  matchCount: number
  ...
  if (!response.ok) {
    console.error("Error retrieving memory:", response)
    return []
  }

  const { results } = (await response.json()) as {
    results: ChatSummaryMatch[]
  }
  ...
  const fallbackResponse = await fetch("/api/local/chat-summaries")

  if (!fallbackResponse.ok) {
    return []
  }

  const allSummaries = (await fallbackResponse.json()) as Array<{
    id: string
    chat_id: string
    workspace_id: string
    summary: string
  ...
  const keywordMatches = allSummaries
    .map(item => ({
      ...item,
      similarity: keywordMatchScore(userInput, item.summary)
    }))
    .filter(item => item.similarity > 0)
    .sort((a, b) => b.similarity - a.similarity)
    .slice(0, matchCount)

  return keywordMatches as ChatSummaryMatch[]
}
      ...
      chunk => {
        ...
        try {
          const contentToAdd = isHosted
            ? chunk
            : // Non-hosted streaming endpoint returns newline-separated JSON
              // objects. A chunk may contain multiple objects.
              chunk
                .trimEnd()
                .split("\n")
                .reduce(

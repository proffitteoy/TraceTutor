...
### 1.1 已落地的卡片化 Chat 主界面

聊天详情当前以 `ChatTree` 空间画布承载，不再把一条聊天限制为单列页面。每个历史聊天无损迁移为独立树的根卡片；侧栏仍只展示根卡片，消息、附件、模型配置和知识检索链路保持原结构。

```text
根卡片
  ├─ 深入子卡：助手文本选区 → 右键 Ask → 引用快照
  ├─ 发散卡：来源标题 + 精简摘要
  └─ 历史分支卡：复制指定轮次以前的消息与附件关联
```

画布使用 `@xyflow/react` 管理节点、连线、平移和缩放。只有活动卡片挂载完整聊天 UI，其余卡片只显示标题、摘要和分支数。卡片正文保留原生文本选择，拖动只允许从标题栏开始；活动卡通过 `?card=<id>` 恢复。

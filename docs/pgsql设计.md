...
注意：模型不能直接指定 `active`。API 必须先校验题目 Schema 和标签白名单，并依赖数据库确认主答案、主解析、解析步骤、主知识点、主方法与最新批准记录全部存在；失败时整个事务回滚。
...
必须带 session / workflow 来源
通过自动质量校验后写入 approved 证据并激活
只有事务成功提交后才能被当前会话和正式召回引用
```

只有在通过上述明确质量校验后，才允许进入：

```text
question_asset.status = active
```

否则事务必须回滚，避免出现一个典型业务问题：

...
- `apps/iris`：当前一键启动链路使用的数学做题工作台，可浏览 66 道已批准题目、提交答案、请求提示、继续追问并渲染真实 Agent 判题结果；它仍不直接访问数据库。
- 做题工作台在每道题内显示 SQLite 持久化的本人作答历史；“新建对话”创建新的会话窗口，但保留题目和历史记录。
...
重复执行时，脚本会校验 55432 上 PostgreSQL 的真实 `data_directory`；确认是项目实例后直接复用，并在重新构建前重启已登记的 API 与 Iris，避免旧进程继续提供旧页面。
